...
void liu::vertex_array::add_buffer(liu::buffer_target target) {
  if (buffers.count(target) == 0) {
    ...
    debug("Added a buffer {}.", target);
  } else {
    warn("Trying to add a existing buffer {}", target);
  }
}
...
void liu::vertex_array::draw(liu::draw_mode mode, int from, int count) { glDrawArrays(static_cast<std::uint32_t>(mode), from, count); }
...
void liu::vertex_array::draw_index(liu::draw_mode mode, int count, liu::index_type type) {
  if (buffers.count(liu::buffer_target::ELEMENT_ARRAY_BUFFER) != 0) {
    glDrawElements(static_cast<std::uint32_t>(mode), count, static_cast<std::uint32_t>(type), nullptr);
  } else {
    warn("Invalid draw call to draw_index with ELEMENT_ARRAY_BUFFER not enabled.");
  }
}
